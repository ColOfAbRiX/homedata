...
final class TimeSlots[A] private (val resolution: FiniteDuration, val store: InnerStore[A]):
  ...
  def toSlottedSortedMap(from: OffsetDateTime, to: OffsetDateTime): SortedMap[OffsetDateTime, Vector[A]] =
    store.filter { case (t, _) => t >= from && t < to }

  val toSlottedSortedMap: SortedMap[OffsetDateTime, Vector[A]] =
    store
