...
object EntityEncoders:
  ...
  def jsonArrayEncoder[F[_], A](
    prefix: String = "[",
    suffix: String = "]",
    delimiter: String = ",",
  )(implicit
    encoder: EntityEncoder[F, A],
  ): EntityEncoder[F, fs2.Stream[F, A]] = {
    ...
    new EntityEncoder[F, fs2.Stream[F, A]] {
      override def toEntity(stream: fs2.Stream[F, A]): Entity[F] =
        Entity(
          `[` ++ stream.map(encoder.toEntity(_).body).intersperse(`,`).flatten ++ `]`,
        )
